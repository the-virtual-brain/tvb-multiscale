...
class HeadPlotter(BasePlotter):
    ...
    def plot_tvb_connectivity(self, connectivity, num="weights", order_by=None, plot_hinton=False, plot_tracts=True):
        ...
        if plot_hinton:
            ...
            # weights matrix
            weights_figure = pyplot.figure(num="Connectivity weights", figsize=self.config.figures.NOTEBOOK_SIZE)
        ...
        if plot_tracts:
            ...
            tracts_figure = pyplot.figure(num="Tracts' lengths", figsize=self.config.figures.NOTEBOOK_SIZE)
