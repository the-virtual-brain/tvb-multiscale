...
import numpy
from tvb.simulator.plot.time_series_plotter import TVBTimeSeriesPlotter
...
class TimeSeriesPlotter(TVBTimeSeriesPlotter):
    ...
    def plot_time_series(self, time_series, mode="ts", subplots=None, special_idx=[], subtitles=[],
        ...
        if isinstance(time_series, TimeSeries):
            ...
            super(TimeSeriesPlotter, self).plot_time_series(time_series, mode, subplots, special_idx, subtitles,
                                                            offset, title, figure_name, figsize, **kwargs)
