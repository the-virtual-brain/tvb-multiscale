...
class CerebBuilder(NESTNetworkBuilder):
    ...
    def __init__(self, tvb_simulator=dict(), spiking_nodes_inds=list(),
                 ...
                 pops_to_nodes_inds=dict(), path_to_network_source_file=""):
        ...
        self.Erev_exc = 0.0  # [mV]	#[Cavallari et al, 2014]
        self.Erev_inh = -80.0  # [mV]
        # tau_exc for pc is for pf input; tau_exc for goc is for mf input; tau_exc for mli is for pf input
        self.tau_exc = {'golgi': 0.23, 'granule': 5.8, 'purkinje': 1.1, 'basket': 0.64, 'stellate': 0.64, 'dcn': 1.0,
                        'dcnp': 3.64, 'dcnGlyI': 1.0,
                        'io': 1.0}
        self.tau_inh = {'golgi': 10.0, 'granule': 13.61, 'purkinje': 2.8, 'basket': 2.0, 'stellate': 2.0, 'dcn': 0.7,
                        'dcnp': 1.14, 'dcnGlyI': 1.0, 'io': 60.0}
        self.tau_exc_cfpc = 0.4
        self.tau_exc_pfgoc = 0.5
        self.tau_exc_cfmli = 1.2
        ...
        self.neuron_param = {
            'golgi_cell': {'t_ref': 2.0, 'C_m': 145.0, 'tau_m': 44.0, 'V_th': -55.0, 'V_reset': -75.0, 'Vinit': -62.0,
                           'E_L': -62.0, 'Vmin': -150.0, 'lambda_0': 1.0, 'tau_V': 0.4, 'I_e': 16.214,
                           'kadap': 0.217, 'k1': 0.031, 'k2': 0.023, 'A1': 259.988, 'A2': 178.01,
                           'E_rev1': self.Erev_exc, 'E_rev2': self.Erev_inh,
                           'E_rev3': self.Erev_exc, 'E_rev4': self.Erev_exc,
                           'tau_syn1': self.tau_exc['golgi'], 'tau_syn2': self. tau_inh['golgi'],
                           'tau_syn3': self.tau_exc_pfgoc, 'tau_syn4': self.tau_exc['golgi']},
            'granule_cell': {'t_ref': 1.5, 'C_m': 7.0, 'tau_m': 24.15, 'V_th': -41.0, 'V_reset': -70.0, 'Vinit': -62.0,
                             'E_L': -62.0, 'Vmin': -150.0, 'lambda_0': 1.0, 'tau_V': 0.3, 'I_e': -0.888,
                             'kadap': 0.022, 'k1': 0.311, 'k2': 0.041, 'A1': 0.01, 'A2': -0.94,
                             'E_rev1': self.Erev_exc, 'E_rev2': self.Erev_inh, 'E_rev3': self.Erev_exc,
                             'tau_syn1': self.tau_exc['granule'], 'tau_syn2': self.tau_inh['granule'],
                             'tau_syn3': self.tau_exc['granule']},
            'purkinje_cell': {'t_ref': 0.5, 'C_m': 334.0, 'tau_m': 47.0, 'V_th': -43.0, 'V_reset': -69.0,
                              'Vinit': -59.0, 'E_L': -59.0, 'lambda_0': 4.0, 'tau_V': 3.5, 'I_e': self.Ie_pc,
                              'kadap': 1.492, 'k1': 0.1950, 'k2': 0.041, 'A1': 157.622, 'A2': 172.622,
                              'E_rev1': self.Erev_exc, 'E_rev2':self. Erev_inh, 'E_rev3': self.Erev_exc,
                              'tau_syn1': self.tau_exc['purkinje'], 'tau_syn2': self.tau_inh['purkinje'],
                              'tau_syn3': self.tau_exc_cfpc},
            'basket_cell': {'t_ref': 1.59, 'C_m': 14.6, 'tau_m': 9.125, 'V_th': -53.0, 'V_reset': -78.0, 'Vinit': -68.0,
                            'E_L': -68.0, 'lambda_0': 1.8, 'tau_V': 1.1, 'I_e': 3.711,
                            'kadap': 2.025, 'k1': 1.887, 'k2': 1.096, 'A1': 5.953, 'A2': 5.863,
                            'E_rev1': self.Erev_exc, 'E_rev2': self.Erev_inh,  'E_rev3': self.Erev_exc,
                            'tau_syn1': self.tau_exc['basket'], 'tau_syn2': self.tau_inh['basket'],
                            'tau_syn3': self.tau_exc_cfmli},
            'stellate_cell': {'t_ref': 1.59, 'C_m': 14.6, 'tau_m': 9.125, 'V_th': -53.0, 'V_reset': -78.0,
                              'Vinit': -68.0, 'E_L': -68.0, 'lambda_0': 1.8, 'tau_V': 1.1, 'I_e': 3.711,
                              'kadap': 2.025, 'k1': 1.887, 'k2': 1.096, 'A1': 5.953, 'A2': 5.863,
                              'E_rev1': self.Erev_exc, 'E_rev2': self.Erev_inh, 'E_rev3': self.Erev_exc,
                              'tau_syn1': self.tau_exc['basket'], 'tau_syn2': self.tau_inh['basket'],
                              'tau_syn3': self.tau_exc_cfmli},
            'dcn_cell_glut_large': {'t_ref': 1.5, 'C_m': 142.0, 'tau_m': 33.0, 'V_th': -36.0, 'V_reset': -55.0,
                                    'Vinit': -45.0, 'E_L': -45.0, 'lambda_0': 3.5, 'tau_V': 3.0, 'I_e': 75.385,
                                    'kadap': 0.408, 'k1': 0.697, 'k2': 0.047, 'A1': 13.857, 'A2': 3.477,
                                    'E_rev1': self.Erev_exc, 'E_rev2': self.Erev_inh, 'E_rev3': self.Erev_exc,
                                    'tau_syn1': self.tau_exc['dcn'], 'tau_syn2': self.tau_inh['dcn']},
            'dcn_cell_GABA': {'t_ref': 3.0, 'C_m': 56.0, 'tau_m': 56.0, 'V_th': -39.0, 'V_reset': -55.0, 'Vinit': -40.0,
                              'E_L': -40.0,  'lambda_0': 0.9, 'tau_V': 1.0, 'I_e': 2.384,
                              'kadap': 0.079, 'k1': 0.041, 'k2': 0.044, 'A1': 176.358, 'A2': 176.358,
                              'E_rev1': Erev_exc, 'E_rev2': Erev_inh, 'E_rev3': Erev_exc, 'tau_syn1': tau_exc['dcnp'],
                              'tau_syn2': tau_inh['dcnp']},
            'dcn_cell_Gly-I': {'t_ref': 3.0, 'C_m': 56.0, 'tau_m': 56.0, 'V_th': -39.0, 'V_reset': -55.0,
                               'Vinit': -40.0, 'E_L': -40.0, 'lambda_0': 0.9, 'tau_V': 1.0, 'I_e': 2.384,
                               'kadap': 0.079, 'k1': 0.041, 'k2': 0.044, 'A1': 176.358, 'A2': 176.358,
                               'E_rev1': Erev_exc, 'E_rev2': Erev_inh, 'E_rev3': Erev_exc,
                               'tau_syn1': tau_exc['dcnGlyI'],
                               'tau_syn2': tau_inh['dcnGlyI']},
            'io_cell': {'t_ref': 1.0, 'C_m': 189.0, 'tau_m': 11.0, 'V_th': -35.0, 'V_reset': -45.0,
                        'Vinit': -45.0, 'E_L': -45.0, 'lambda_0': 1.2, 'tau_V': 0.8, 'I_e': -18.101,
                        'kadap': 1.928, 'k1': 0.191, 'k2': 0.091, 'A1': 1810.93, 'A2': 1358.197,
                        'E_rev1': Erev_exc, 'E_rev2': Erev_inh, 'E_rev3': Erev_exc, 'tau_syn1': tau_exc['io'],
                        'tau_syn2': tau_inh['io']}}

        # Connection weights
        self.conn_weights = \
            {'mossy_to_glomerulus': 1.0, 'ascending_axon_to_golgi': 1.5, 'ascending_axon_to_purkinje': 1.08,
             ...
             'parallel_fiber_to_stellate': 0.042, 'stellate_to_purkinje': 0.85,
             'stellate_to_stellate': 0.109,
             'dcn_GABA_to_io': 0.00000001, 'io_to_dcn_GABA': 0.2, 'io_to_dcn_glut_large': 0.1, 'io_to_purkinje': 350.0,
              'mossy_to_dcn_Gly-I': 0.8, 'mossy_to_dcn_glut_large': 0.5,
             'purkinje_to_dcn_GABA': 0.00418, 'purkinje_to_dcn_Gly-I': 0.2, 'purkinje_to_dcn_glut_large': 0.5}
        ...
        self.conn_delays = {
            ...
            'glomerulus_to_golgi': 4.0, 'glomerulus_to_granule': 4.0,
            'golgi_to_granule': 2.0, 'golgi_to_golgi': 1.0,
             'parallel_fiber_to_basket': 5.0, 'parallel_fiber_to_golgi': 5.0,
            'parallel_fiber_to_purkinje': 5.0, 'parallel_fiber_to_stellate': 5.0,
            'stellate_to_purkinje': 5.0, 'stellate_to_stellate': 1.0,
             'NC_dcn_Gly-I_to_golgi': 1.0, 'NC_dcn_glut_large_to_golgi': 1.0, 'NC_dcn_glut_large_to_granule': 1.0,
            'dcn_GABA_to_io': 20.0,
            'io_to_dcn_GABA': 5.0, 'io_to_basket': {"mu": 70.0, "sigma": 10.0},
            'io_to_stellate': {"mu": 70.0, "sigma": 10.0}, 'io_to_purkinje': 4.0, 'io_to_dcn_glut_large': 4.0,
            'mossy_to_dcn_Gly-I': 4.0, 'mossy_to_dcn_glut_large': 4.0,
            'purkinje_to_dcn_GABA': 4.0, 'purkinje_to_dcn_Gly-I': 4.0, 'purkinje_to_dcn_glut_large': 4.0}

        # Connection receptors
        self.conn_receptors = {
            'ascending_axon_to_golgi': 3, 'ascending_axon_to_purkinje': 1,
            'basket_to_purkinje': 2, 'basket_to_basket': 2,
            'glomerulus_to_golgi': 1, 'glomerulus_to_granule': 1,
            'golgi_to_granule': 2, 'golgi_to_golgi': 2,
            'parallel_fiber_to_basket': 1, 'parallel_fiber_to_golgi': 3,
            'parallel_fiber_to_purkinje': 1, 'parallel_fiber_to_stellate': 1,
            'stellate_to_purkinje': 2, 'stellate_to_stellate': 2,
            'NC_dcn_Gly-I_to_golgi': 4, 'NC_dcn_glut_large_to_golgi': 4, 'NC_dcn_glut_large_to_granule': 3,
            'dcn_GABA_to_io': 2, 'io_to_basket': 3, 'io_to_dcn_GABA': 1, 'io_to_dcn_glut_large': 1,
            'io_to_purkinje': 3, 'io_to_stellate': 3,
            'mossy_to_dcn_Gly-I': 1, 'mossy_to_dcn_glut_large': 1,
            'purkinje_to_dcn_GABA': 2, 'purkinje_to_dcn_Gly-I': 2, 'purkinje_to_dcn_glut_large': 2}
        ...
        self.STIMULUS = True
        self.TOT_DURATION = 400.  # ms
        self.STIM_MF_START = 140.  # beginning of stimulation to MFs
        self.STIM_MF_END = 300.  # end of stimulation to MFs
        self.STIM_MF_FREQ = 40.  # MF Frequency in Hz
        self.STIM_IO_START = 290.  # beginning of stimulation to IO
        self.STIM_IO_END = 300.  # end of stimulation to IO
        self.STIM_IO_FREQ = 500.  # IO Frequency in Hz
        self.BACKGROUND_FREQ = 4.  # Background Frequency in Hz

        self.ordered_populations_labels = ['mossy_fibers', 'glomerulus', "granule_cell", "golgi_cell",
                                           'io_cell', "basket_cell", "stellate_cell", "purkinje_cell",
                                           'dcn_cell_GABA', 'dcn_cell_Gly-I', 'dcn_cell_glut_large']
        self.start_id_scaffold = list()
