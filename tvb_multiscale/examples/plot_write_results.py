...
def plot_results(results, simulator, population_sizes, transient=0.0,
    ...
    if transient:
        ...
        exclude_times = None
    t = source_ts.time
    writer.write_tvb_to_h5(source_ts, os.path.join(config.out.FOLDER_RES, source_ts.title)+".h5")
    ...
    rates = \
        spiking_network.compute_spikes_rates(mode="per_neuron", populations_devices=None, regions=None,
                                             ...
                                             spikes_kernel_width=0.1,  # spikes_kernel_n_intervals=10,
    ...
    if nest_ts.size > 0:
        ...
        # Remove the Neuron dimension from the time series for the mean-field time series
        ...
        writer.write_tvb_to_h5(ts, os.path.join(config.out.FOLDER_RES, ts.title)+".h5")
