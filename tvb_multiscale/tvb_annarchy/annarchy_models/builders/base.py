...
from tvb_multiscale.tvb_annarchy.annarchy_models.builders.annarchy_factory import \
    load_annarchy, compile, create_conn_spec, create_device, connect_device
...
class ANNarchyModelBuilder(SpikingModelBuilder):
    ...
    def _configure_nest_kernel(self):
        self.annarchy_instance.ResetKernel()  # This will restart NEST!
        self._update_spiking_dt()
        self._update_default_min_delay()
        self.annarchy_instance.set_verbosity(self.config.NEST_VERBOCITY)  # don't print all messages from NEST
        self.annarchy_instance.SetKernelStatus({"resolution": self.spiking_dt, "print_time": self.config.NEST_PRINT_TIME})
    ...
    def _compile(self, module):
        ...
        try:
            ...
            self.annarchy_instance.Install(module)
            ...
            compile(module_name, recompile=False, config=self.config)
            # and now install it...
    ...
    def compile_install_nest_modules(self, modules_to_install):
        if len(modules_to_install) > 0:
            ...
            while len(modules_to_install) > 0:
                self._compile(modules_to_install.pop())

    def confirm_compile_install_nest_models(self, models):
        nest_models = self.annarchy_instance.Models()
    ...
    def min_delay(self):
        try:
            return self.annarchy_instance.GetKernelStatus("min_delay")
        except:
