...
def Calc_Zs_Shrink_param(param,default,iG,TrainSamples):
...
if __name__ == "__main__":
    ...
    for iG in range(3):
        ...
        for k,v in Default.items():
            param=k
            default=v
            Zg,Sg,Zgst=Calc_Zs_Shrink_param(param,default,iG,TrainSamples)
            print('for param :',param)
            print('Zscore :',Zg)
            print('Shrinkage:',Sg)
            print('Zscore std :',Zgst)
            plt.plot(TrainSamples,Sg,color=colors[c] ,label='Shrinkage_iG%g_%s'%(iG,param))
            plt.legend(loc='upper left')
            c=c+1
            #plt.plot(TrianSamples,Sg,color,label='Shrinkage_iG%g_%s'%(i
            plt.xlabel("TrainSamples")
            plt.ylabel("Shrinkage")
    plt.savefig(os.path.join(config.figures.FOLDER_FIGURES,'Shrinkage_IS_FIC_FICSPLIT_2fev.png'))             
