...
def basal_ganglia_izhikevich_example(**kwargs):
    ...
    tvb_annarchy_model_builder.GPe_proxy_inds = np.array([0, 1])
    tvb_annarchy_model_builder.GPi_proxy_inds = np.array([2, 3])
    tvb_annarchy_model_builder.STN_proxy_inds = np.array([4, 5])
    tvb_annarchy_model_builder.Striatum_proxy_inds = np.array([6, 7])
    tvb_annarchy_model_builder.Thal_proxy_inds = np.array([8, 9])
    ...
    #                       [tvb_annarchy_model_builder.E_proxy_inds, tvb_annarchy_model_builder.Striatum_proxy_inds],
    #                       ["E",                                 "ISN"])):
    ...
    #                   [tvb_annarchy_model_builder.E_proxy_inds,
    #                    tvb_annarchy_model_builder.I_proxy_inds,
    #                    tvb_annarchy_model_builder.Striatum_proxy_inds],
    #                    ["E", "I", "ISN"])):
    #     spikeNet_to_tvb_interfaces.append(
    #         {"voi": ("S", "R"), "populations": src_pop,
    #          "transformer": spikeNet_to_tvb_transformer,
    #          "transformer_params": {"scale_factor": np.array([1.0]) / tvb_annarchy_model_builder.N_E,
    #                                 "integrator":
    ...
    return main_example(LinearReducedWongWangExcIO, model_params,
                        ...
                        tvb_annarchy_model_builder, tvb_to_spikeNet_interfaces, spikeNet_to_tvb_interfaces,
