...
def plot_result(PSE_params, result, name, path):
    ...
    for i_w1, w1 in enumerate(arr.shape[1]):
        for i_w2, w2 in enumerate(arr.shape[2]):
...
class PSETVBNESTWorkflow(PSEWorkflowBase):
    ...
    def __init__(self, n_nodes=2, tvb_nodes=[0], nest_nodes=[1], SC=None,
        ...
        self._plot_results = ["rate", "spike rate", "Pearson", "Spearman",
                              "spike Pearson", "spike Spearman", "spike train"]
        self._corr_results = ["Pearson", "Spearman", "spike Pearson", "spike Spearman", "spike train"]
    ...
    def prepare_PSE_results(self):
        Ntvb_reg_shape = (self.n_tvb_nodes,) + self.pse_shape
    ...
    def run(self):
        ...
        if self.n_nodes > 2:
            ...
            if self.n_nest_nodes > 2:
                self.SC_nest = self.workflow.simulator.connectivity.weights[self.nest_nodes][:, self.nest_nodes]
    ...
    def load_PSE_2D(self, **kwargs):
        ...
        for i_w1, wTVB in enumerate(self.PSE["params"]["w+TVB"]):
            for i_w2, wNEST in enumerate(self.PSE["params"]["w+NEST"]):
                path = self.res_path.replace(".h5", "_w+TVB%g_w+NEST%g.h5" % (wTVB, wNEST))
                try:
                ...
                for n in ["TVB", "NEST"]:
                    ...
                    for i_pop, pop in enumerate(self.PSE["results"][rate].keys()):
                        ...
                        self.PSE["results"][rate][pop][:, i_w1, i_w2] = PSE["results"][rate][pop].item()
                for corr in self._corr_results:
                    self.PSE["results"][corr]["EE"][:, i_w1, i_w2] = PSE["results"][corr]["EE"].item()
                    try:
                        self.PSE["results"][corr]["FC-SC"][:, i_w1, i_w2] = PSE["results"][corr]["FC-SC"].item()
                    except:
                        pass
