...
* Recordables map
* ---------------------------------------------------------------- */
nest::RecordablesMap<iaf_cond_nmda_deco2014> iaf_cond_nmda_deco2014::recordablesMap_;

namespace nest
{
  // Override the create() method with one call to RecordablesMap::insert_()
  // for each quantity to be recorded.
  template <> void RecordablesMap<iaf_cond_nmda_deco2014>::create(){
  // use standard names whereever you can for consistency!  

  insert_("s_bound", &iaf_cond_nmda_deco2014::get_s_bound);

  insert_("emmited_spike", &iaf_cond_nmda_deco2014::get_emmited_spike);

  insert_("I_leak", &iaf_cond_nmda_deco2014::get_I_leak);

  insert_("I_AMPA_ext", &iaf_cond_nmda_deco2014::get_I_AMPA_ext);

  insert_("I_AMPA_rec", &iaf_cond_nmda_deco2014::get_I_AMPA_rec);

  insert_("I_NMDA", &iaf_cond_nmda_deco2014::get_I_NMDA);

  insert_("I_GABA", &iaf_cond_nmda_deco2014::get_I_GABA);

  insert_("x_NMDA", &iaf_cond_nmda_deco2014::get_x_NMDA);

  insert_("s", &iaf_cond_nmda_deco2014::get_s);

  insert_("s_AMPA_ext", &iaf_cond_nmda_deco2014::get_s_AMPA_ext);

  insert_("s_AMPA_rec", &iaf_cond_nmda_deco2014::get_s_AMPA_rec);

  insert_("s_NMDA", &iaf_cond_nmda_deco2014::get_s_NMDA);

  insert_("s_GABA", &iaf_cond_nmda_deco2014::get_s_GABA);

  insert_("V_m", &iaf_cond_nmda_deco2014::get_V_m);
  }
}
 ...
 * is a part of the iaf_cond_nmda_deco2014's constructor.
 * ---------------------------------------------------------------- */
iaf_cond_nmda_deco2014::Parameters_::Parameters_(){}

iaf_cond_nmda_deco2014::State_::State_(){}

/* ----------------------------------------------------------------
* Parameter and state extractions and manipulation functions
* ---------------------------------------------------------------- */

iaf_cond_nmda_deco2014::Buffers_::Buffers_(iaf_cond_nmda_deco2014 &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}

iaf_cond_nmda_deco2014::Buffers_::Buffers_(const Buffers_ &, iaf_cond_nmda_deco2014 &n):
  logger_(n), spike_inputs_( std::vector< nest::RingBuffer >( SUP_SPIKE_RECEPTOR - 1 ) ), __s( 0 ), __c( 0 ), __e( 0 ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}
 ...
 * ---------------------------------------------------------------- */
iaf_cond_nmda_deco2014::iaf_cond_nmda_deco2014():Archiving_Node(), P_(), S_(), B_(*this)
{
  ...
  S_.ode_state[State_::x_NMDA] = 0.0; // as 1 / ms
...
void iaf_cond_nmda_deco2014::init_state_(const Node& proto){
  const iaf_cond_nmda_deco2014& pr = downcast<iaf_cond_nmda_deco2014>(proto);
  S_ = pr.S_;
}



extern "C" inline int iaf_cond_nmda_deco2014_dynamics(double, const double ode_state[], double f[], void* pnode){
  typedef iaf_cond_nmda_deco2014::State_ State_;
  // get access to node so we can almost work as in a member function
  assert( pnode );
  const iaf_cond_nmda_deco2014& node = *( reinterpret_cast< iaf_cond_nmda_deco2014* >( pnode ) );
  ...
  f[State_::x_NMDA] = -(ode_state[State_::x_NMDA]) / node.get_tau_NMDA_rise() + emmited_spike / pow(1.0, 2);
  f[State_::s] = -(s_bound) / node.get_tau_NMDA_decay() + node.get_alpha() * ode_state[State_::x_NMDA] * (1 - s_bound);
  ...
  f[State_::V_m] = (-(I_leak) - I_AMPA_ext - I_AMPA_rec - I_NMDA - I_GABA + node.get_I_e() + 1.0 * (node.B_.currents_grid_sum_)) / node.get_C_m();
  return GSL_SUCCESS;
}



void iaf_cond_nmda_deco2014::init_buffers_(){
  ...
  B_.__sys.function = iaf_cond_nmda_deco2014_dynamics;
...
void iaf_cond_nmda_deco2014::calibrate(){
  B_.logger_.init();
 ...
 *
 */
void iaf_cond_nmda_deco2014::update(nest::Time const & origin,const long from, const long to){
  double __t = 0;
...
void iaf_cond_nmda_deco2014::handle(nest::DataLoggingRequest& e){
  B_.logger_.handle(e);
}


void iaf_cond_nmda_deco2014::handle(nest::SpikeEvent &e){
...
void iaf_cond_nmda_deco2014::handle(nest::CurrentEvent& e){
  assert(e.get_delay_steps() > 0);
