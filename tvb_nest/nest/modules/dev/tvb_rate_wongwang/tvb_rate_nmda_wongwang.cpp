...
nest::RecordablesMap< tvbnest::tvb_rate_nmda_wongwang > tvbnest::tvb_rate_nmda_wongwang::recordablesMap_;
...
template <>
void
RecordablesMap< tvbnest::tvb_rate_nmda_wongwang >::create()
{
  insert_( names::x, &tvbnest::tvb_rate_nmda_wongwang::get_x_ );
  insert_( "noise_x", &tvbnest::tvb_rate_nmda_wongwang::get_noise_x_ );
  insert_( names::S, &tvbnest::tvb_rate_nmda_wongwang::get_S_ );
  insert_( "noise_S", &tvbnest::tvb_rate_nmda_wongwang::get_noise_S_ );
  insert_( names::V_m, &tvbnest::tvb_rate_nmda_wongwang::get_V_m_ );
  insert_( "noise_V_m", &tvbnest::tvb_rate_nmda_wongwang::get_noise_V_m_ );
  insert_( "s_AMPA_ext", &tvbnest::tvb_rate_nmda_wongwang::get_s_AMPA_ext );
  insert_( "s_AMPA_rec", &tvbnest::tvb_rate_nmda_wongwang::get_s_AMPA_rec );
  insert_( "s_NMDA", &tvbnest::tvb_rate_nmda_wongwang::get_s_NMDA );
  insert_( "s_GABA", &tvbnest::tvb_rate_nmda_wongwang::get_s_GABA );
  insert_( "I_leak", &tvbnest::tvb_rate_nmda_wongwang::get_I_leak);
  insert_( "I_AMPA_ext", &tvbnest::tvb_rate_nmda_wongwang::get_I_AMPA_ext );
  insert_( "I_AMPA_rec", &tvbnest::tvb_rate_nmda_wongwang::get_I_AMPA_rec );
  insert_( "I_NMDA", &tvbnest::tvb_rate_nmda_wongwang::get_I_NMDA );
  insert_( "I_GABA", &tvbnest::tvb_rate_nmda_wongwang::get_I_GABA );
  insert_( names::spike, &tvbnest::tvb_rate_nmda_wongwang::get_spike_ );
  insert_( names::I_syn, &tvbnest::tvb_rate_nmda_wongwang::get_I_syn_ );
}
} // namespace
...
tvbnest::tvb_rate_nmda_wongwang::Parameters_::Parameters_()
  : V_th_( -50.0 ) // mV
  , V_reset_( -55.0 ) // mV
  , E_L_( -70.0 ) // mV
  , E_ex_( 0.0 ) // mV
  , E_in_( -70.0 ) // mV
  , t_ref_( 2.0 )   // ms
  , tau_rise_NMDA_( 2.0 )   // ms
  , tau_decay_NMDA_( 100.0 )   // ms
  , C_m_( 500.0 ) // pF
  , g_L_( 25.0 ) // nS
  , g_AMPA_ext_( 3.37 ) // nS
  ...
  , I_e_( 0.0 ) // pA
  , spike_amplitude_( 2.0 ) // real unitless default set equal to tau_rise_NMDA
  , sigma_( 0.01 )
  , sigma_x_( 1.0 )
  , sigma_S_( 1.0 )
  , sigma_V_m_( 1.0 )
...
tvbnest::tvb_rate_nmda_wongwang::State_::State_()
  : x_( 0.0 )
  , noise_x_( 0.0 )
  , S_( 0.0 )
  , noise_S_( 0.0 )
...
void
tvbnest::tvb_rate_nmda_wongwang::Parameters_::get(
  DictionaryDatum& d ) const
{
  ...
  def< double >( d, names::tau_rise_NMDA, tau_rise_NMDA_ );
  def< double >( d, names::tau_decay_NMDA, tau_decay_NMDA_ );
...
void
tvbnest::tvb_rate_nmda_wongwang::Parameters_::set(
  const DictionaryDatum& d )
{
  ...
  updateValue< double >( d, names::tau_rise_NMDA, tau_rise_NMDA_ );
  ...
  updateValue< double >( d, "sigma_x", sigma_x_ );
  ...
  {
    LOG( M_WARNING,
      "tvb_rate_nmda_wongwang::Parameters_::set",
  ...
  if ( alpha_ < 0 )
  {
    throw nest::BadProperty( "alpha must be >= 0." );
  }
  if ( beta_ < 0 )
  {
  ...
  if ( sigma_x_ < 0 )
  {
    throw nest::BadProperty( "Noise parameter sigma_x must be >= 0." );
  }
  if ( sigma_S_ < 0 )
  {
...
void
tvbnest::tvb_rate_nmda_wongwang::State_::get(
  DictionaryDatum& d ) const
{
  def< double >( d, names::x, x_ );
  def< double >( d, "noise_x", noise_x_ );
...
void
tvbnest::tvb_rate_nmda_wongwang::State_::set(
  const DictionaryDatum& d )
{
  updateValue< double >( d, names::x, x_ );
  updateValue< double >( d, "noise_x", noise_x_ );
...
tvbnest::tvb_rate_nmda_wongwang::tvb_rate_nmda_wongwang()
  : Archiving_Node()
  , P_()
  , S_()
  , B_( *this )
...
tvbnest::tvb_rate_nmda_wongwang::tvb_rate_nmda_wongwang(
    const tvbnest::tvb_rate_nmda_wongwang& n )
  : Archiving_Node( n )
  , P_( n.P_ )
  , S_( n.S_ )
  , B_( n.B_, *this )
...
tvbnest::tvb_rate_nmda_wongwang::Buffers_::Buffers_(
  tvbnest::tvb_rate_nmda_wongwang& n )
  : logger_( n ),
    delayed_S_inputs_( std::vector< nest::RingBuffer >( SUP_RECEPTOR - 1 ) ),
    instant_S_inputs_( std::vector<std::vector< double >> ( SUP_RECEPTOR - 1 ) ),
    last_y_values( std::vector<std::vector< double >> ( 3 ) ),
    random_numbers( std::vector<std::vector< double >> ( 3 ) ){
  // Initialization of the remaining members is deferred to
  // init_buffers_().
}

tvbnest::tvb_rate_nmda_wongwang::Buffers_::Buffers_( const Buffers_&,
  tvbnest::tvb_rate_nmda_wongwang& n )
  : logger_( n ),
    delayed_S_inputs_( std::vector< nest::RingBuffer >( SUP_RECEPTOR - 1 ) ),
    instant_S_inputs_( std::vector<std::vector< double >> ( SUP_RECEPTOR - 1 ) ),
...
void
tvbnest::tvb_rate_nmda_wongwang::init_state_( const Node& proto )
{
  const tvbnest::tvb_rate_nmda_wongwang& pr = downcast< tvbnest::tvb_rate_nmda_wongwang >( proto );
  S_ = pr.S_;
}

void
tvbnest::tvb_rate_nmda_wongwang::init_buffers_()
{
...
void
tvbnest::tvb_rate_nmda_wongwang::calibrate()
{
  B_.logger_
  ...
  const std::vector<double> tau = {P_.tau_rise_NMDA_, P_.tau_decay_NMDA_, P_.C_m_/ P_.g_L_};

  V_.g_L_E_L_ = P_.g_L_ * P_.E_L_ ;
  V_.alpha_tau_decay_NMDA_ = P_.alpha_ / P_.tau_decay_NMDA_;
  ...
  V_.sigma_ = {P_.sigma_ * P_.sigma_x_, P_.sigma_ * P_.sigma_S_, P_.sigma_ * P_.sigma_V_m_};
...
bool
tvbnest::tvb_rate_nmda_wongwang::update_( Time const& origin,
  const long from,
  const long to,
  const bool called_from_wfr_update )
{
  ...
  for ( long lag = from; lag < to; ++lag )
  {
    ...
    // get noise
    S_.noise_x_ = V_.sigma_[0] * B_.random_numbers[0][ lag ];
    S_.noise_S_ = V_.sigma_[1] * B_.random_numbers[1][ lag ];
    S_.noise_V_m_ = V_.sigma_[2] * B_.random_numbers[2][ lag ];
    // propagate to new time step
    S_.S_ = V_.P1_[1] * new_S[ lag ]
          + V_.P2_[1] * ( - new_S[ lag ]
                          + V_.alpha_tau_decay_NMDA_ * S_.x_ * (1 - new_S[ lag ]))
          + V_.input_noise_factor_[1] * S_.noise_S_ ;
    S_.x_ = V_.P1_[0] * S_.x_
          - V_.P2_[0] * S_.x_
          + V_.input_noise_factor_[0] * S_.noise_x_ ;
    ...
    if ( called_from_wfr_update )
    {
      ...
      wfr_tol_exceeded = wfr_tol_exceeded
        or fabs( S_.x_ - B_.last_y_values[ 0 ][ lag ] ) > wfr_tol
        or fabs( S_.S_ - B_.last_y_values[ 1 ][ lag ] ) > wfr_tol
        or fabs( S_.V_m_ - B_.last_y_values[ 2 ][ lag ] ) > wfr_tol;
      // update last_y_values for next wfr iteration
      B_.last_y_values[ 0 ][ lag ] = S_.x_;
      B_.last_y_values[ 1 ][ lag ] = S_.S_;
      B_.last_y_values[ 2 ][ lag ] = S_.V_m_;
    }
    else
    {
      // rate logging
...
void
tvbnest::tvb_rate_nmda_wongwang::handle(
...
void
tvbnest::tvb_rate_nmda_wongwang::handle( DataLoggingRequest& e )
{
  B_.logger_.handle( e );
}
