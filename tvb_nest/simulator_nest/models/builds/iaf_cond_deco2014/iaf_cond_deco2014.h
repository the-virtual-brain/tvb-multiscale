...
private:
  ...
  */
  struct State_{
    ...
    double s_AMPA_ext_bounded;

    double s_AMPA_rec_bounded;

    double s_NMDA_bounded;

    double s_GABA_bounded;

    double I_leak;

    //!  * convolve(s_AMPA_ext, spikesExc_AMPA_ext)
    double I_AMPA_ext;

    //!  * convolve(s_AMPA_rec, spikesExc_AMPA_rec)
    double I_AMPA_rec;

    //!  * convolve(s_NMDA, spikesExc_NMDA)
    double I_NMDA;

    //!  * convolve(s_GABA, spikesInh_GABA)
    double I_GABA;
        State_();
  };
  ...
  */
  ...
  inline double get_I_AMPA_ext() const {
    return P_.g_AMPA_ext / 1.0 * (S_.ode_state[State_::V_m] - P_.E_ex);
  }

  inline double get_I_AMPA_rec() const {
    return P_.g_AMPA_rec / 1.0 * (S_.ode_state[State_::V_m] - P_.E_ex);
  }

  inline double get_I_NMDA() const {
    return P_.g_NMDA / 1.0 * (S_.ode_state[State_::V_m] - P_.E_ex);
  }

  inline double get_I_GABA() const {
    return P_.g_GABA / 1.0 * (S_.ode_state[State_::V_m] - P_.E_in);
  }
