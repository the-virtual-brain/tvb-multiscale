...
extern "C" inline int iaf_cond_deco2014_dynamics(double, const double ode_state[], double f[], void* pnode){
  ...
  f[State_::V_m] = (-((node.get_g_m() * (ode_state[State_::V_m] - node.get_E_L()))) - (node.get_g_AMPA_ext() / 1.0 * (ode_state[State_::V_m] - node.get_E_ex())) - (node.get_g_AMPA_rec() / 1.0 * (ode_state[State_::V_m] - node.get_E_ex())) - (node.get_g_NMDA() / 1.0 * (ode_state[State_::V_m] - node.get_E_ex())) / (1 + node.get_lamda_NMDA() * std::exp(-(node.get_beta()) * (ode_state[State_::V_m] / 1.0*1.0))) - (node.get_g_GABA() / 1.0 * (ode_state[State_::V_m] - node.get_E_in())) + node.get_I_e() + node.B_.currents_grid_sum_) / node.get_C_m();
  return GSL_SUCCESS;
}
...
void iaf_cond_deco2014::update(nest::Time const & origin,const long from, const long to){
  ...
  for ( long lag = from ; lag < to ; ++lag ) {
    ...
    S_.ode_state[State_::s_AMPA_ext] = bound(S_.ode_state[State_::s_AMPA_ext] / 1.0) * 1.0 + 1.0 * (B_.spikesExc_AMPA_ext_grid_sum_);
    

    S_.ode_state[State_::s_AMPA_rec] = bound(S_.ode_state[State_::s_AMPA_rec] / 1.0) * 1.0 + 1.0 * (B_.spikesExc_AMPA_rec_grid_sum_);
    

    S_.ode_state[State_::x_NMDA] = bound(S_.ode_state[State_::x_NMDA]) * 1.0 / 1.0 + B_.spikesExc_NMDA_grid_sum_ / 1.0;
    

    S_.ode_state[State_::s_GABA] = bound(S_.ode_state[State_::s_GABA] / 1.0) * 1.0 + 1.0 * (B_.spikesInh_GABA_grid_sum_);
